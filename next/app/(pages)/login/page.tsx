...
import { _IUser, IApiResponse } from "../../../../server/type";
...
const UserAuth = () => {
  ...
  const send = async () => {
    const f = await fetch(API_BASE_URL + "/api/otp/send", {
      method: "post",
  ...
  const verify = async () => {
    const f = await fetch(API_BASE_URL + "/api/otp/verify", {
      method: "post",
  ...
  const deleteAuth = async (refreshPath: "currentPage" | "homePage") => {
    const f = await fetch(API_BASE_URL + "/api/otp/invalidate", {
      method: "delete",
    });
    const response: IApiResponse<null> = await f.json();
