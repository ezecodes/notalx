    ...
    <div className="relative w-full  ">
      ...
      {showDropdown && options.length > 0 && (
        ...
        >
          ...
          <Link href={"/account"} className="text-sm px-4 flex justify-center">
            New Account
          </Link>
        </ul>
      )}
              ...
              <div className="flex h-[35px] justify-between px-2 py-1 border_top gap-x-3 w-full absolute bottom-0 left-0 items-center">
                ...
                <span className="flex items-center gap-x-4">
                  ...
                  <TaskOptionsPopup
                    deleteTask={() => {}}
                    manage={() =>
                      router.push(`/task/${encodeToBase62(row.task.id)}`)
                    }
                  />
                </span>
              </div>
...
export const AuthorisedInfo = ({
  ...
  return (
    <div
      ...
      onClick={() => router.push(clickUrl as any, {})}
    >
      <BsPersonCheck
        className={`${
...
export const SharedHeader = () => {
  ...
  const createNote = async () => {
    try {
      setLoader(true);
      const f = await fetch(API_BASE_URL + "/api/note", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  return (
    ...
    >
      <div
        ...
        onClick={() => router.push(`/${encodeToBase62(note.id)}`)}
      >
