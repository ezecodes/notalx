...
import { fetchAuthUserNotes } from "../_utils";
...
const Provider: FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const fetchNotesSharedWithUser = async () => {
    const f = await fetch(API_BASE_URL + "/api/note/shared");
    const res: IPaginatedResponse<ApiFetchNote> = await f.json();
  ...
  const getOTPExpiry = async () => {
    try {
      const f = await fetch(API_BASE_URL + "/api/otp/expiry");
...
const useMicrophone = () => {
  ...
  const sendAudioToBackend = async () => {
    ...
    try {
      setStatus("analyzing");
      const response = await fetch(API_BASE_URL + "/api/note/audio", {
        method: "POST",
