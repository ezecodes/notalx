...
export const searchUserByName = async (name: string) => {
  const f = await fetch(API_BASE_URL + "/api/user/search?name=" + name);
  return (await f.json()) as IPaginatedResponse<_IUser>;
};

export const fetchNote = async (slug: string) => {
  const f = await fetch(API_BASE_URL + "/api/note/" + slug, {
    headers: {
...
export const fetchUser = async (id: string) => {
  const f = await fetch(API_BASE_URL + "/api/user/" + id);
...
export const fetchAllScheduledTasksForUser = async (): Promise<ITaskList> => {
