...
export async function PopulateTaskParticipants(
  ...
  const rows = await Promise.all(
    find.map(async (i) => await User.findByPkWithCache(i.dataValues.user_id))
  );
...
export async function PopulateNoteCollaborators(note_id: string) {
  const rows: ICollaborator[] = (await NoteCollaborator.findAll({
    where: { note_id },
    raw: true,
  })) as any;
...
export const audioTransciption = async (filePath: string) => {
  ...
  const response: ICloudflareResponse<IWhisperResponse> = await f.json();
